...
def listen_and_transcribe(recognizer, source, cfg):
    print("[VIKI] Ouvindo...")
    try:
        # Aumentamos o timeout levemente, mas agora ela escuta direto sem ajustar ruidos a todo momento
        audio = recognizer.listen(source, timeout=cfg["stt_timeout"], phrase_time_limit=cfg["stt_phrase_time_limit"])
        print("[VIKI] Processando sua fala (STT)...")
        text = recognizer.recognize_google(audio, language=cfg["stt_language"])
        print(f"[VOCE] {text}")
        return text.lower()
...
def main():
    ...
    try:
        ...
        for idx, m in enumerate(mics):
            if cfg["mic_search_string"] in m and qcy_index is None:
                qcy_index = idx  # Salva o primeiro microfone que corresponder

        if qcy_index is not None:
             print(f"\n[VIKI] Sucesso: Encontrei o microfone '{cfg['mic_search_string']}' no índice {qcy_index}! Prendendo áudio nele.")
        else:
             print(f"\n[VIKI] ALERTA: Não consegui achar '{cfg['mic_search_string']}' na lista, usando o padrão do Windows.")
    ...
    # Microfone fica aberto continuadamente e travado no dispositivo configurado
    with sr.Microphone(device_index=qcy_index) as source:
        print(f"\n[VIKI] Calibrando microfone (fique em silêncio por {cfg['ambient_calibration_duration']} segundos)...")
        recognizer.adjust_for_ambient_noise(source, duration=cfg["ambient_calibration_duration"])
        # Trava um limite manualmente máximo sensível caso o ajuste erre pra cima
        if recognizer.energy_threshold > 1000:
            print(f"[DEBUG] O Windows reportou muito ruído. Forçando alta sensibilidade ({cfg['energy_threshold']}).")
        ...
        while True:
            text = listen_and_transcribe(recognizer, source, cfg)
